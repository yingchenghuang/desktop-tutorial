...
art = JSON.parse(File.read(File.join(root, "data/backfill-september-20260901.json"))).fetch("entries")
calls = JSON.parse(File.read(File.join(root, "data/competitions.json"))).fetch("entries").select { |e| e["updated"] == "2026-09-01" }
exhibitions = JSON.parse(File.read(File.join(root, "data/exhibitions.json"))).fetch("entries").select { |e| e["updated"] == "2026-09-01" }
