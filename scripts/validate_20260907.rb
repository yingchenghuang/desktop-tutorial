...
date = "2026-09-07"
now = DateTime.parse("2026-09-07T04:00:00+08:00")
errors = []

day = JSON.parse(File.read(File.join(data, "backfill-september-20260907.json")))
...
calls = comp.fetch("entries").select { |e| e["updated"] == date }
...
shows = ex.fetch("entries").select { |e| e["updated"] == date }
...
manifest = JSON.parse(File.read(File.join(data, "backfill-september-manifest.json")))
errors << "manifest missing today" unless manifest.fetch("files", []).include?("backfill-september-20260907.json")
