...
art = JSON.parse(File.read(File.join(root, "data/backfill-august-20260831.json"))).fetch("entries")
calls = JSON.parse(File.read(File.join(root, "data/competitions.json"))).fetch("entries").select { |e| e["updated"] == "2026-08-31" }
exhibitions = JSON.parse(File.read(File.join(root, "data/exhibitions.json"))).fetch("entries").select { |e| e["updated"] == "2026-08-31" }
